...
uint32_t _network_get_interface_ip(uint32_t peer_addr)
{
	...
	/* in case the node domain name is not set up correctly */
	if (strncmp(hostname, "localhost", 9) == 0) {
		if ((fh = socket(PF_INET,SOCK_DGRAM,IPPROTO_IP)) > 0) {
			struct ifreq my_ifreqs[10];
			struct ifconf my_ifconf;
			
			my_ifconf.ifc_len = sizeof(my_ifreqs);
			my_ifconf.ifc_req = my_ifreqs; 
			if (ioctl(fh,SIOCGIFCONF,&my_ifconf) >= 0) {
				int num = my_ifconf.ifc_len / sizeof(struct ifreq);
				int i;
				
				for(i = 0; i < num; i++) {
					struct sockaddr_in * adr = (struct sockaddr_in *)&my_ifreqs[i].ifr_ifru.ifru_addr;
					if (strncmp(my_ifreqs[i].ifr_name, "eth", 3) == 0) {
						/* printf("ip: %s\n", inet_ntoa((struct in_addr)adr->sin_addr)); */
						close(fh);
						return adr->sin_addr.s_addr;
					}
				}
			}
			printf("Error: getting the correct interface failed!!\n"
				   "If you see this error, please report to KyoungSoo\n");
			close(fh);
		}
	}
